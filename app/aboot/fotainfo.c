...
static int get_fota_info(struct fota_info *info)
{
	...
	unsigned int blocksize = flash_block_size();
	unsigned int crc_val = 0;

	if (info == NULL) {
		dprintf(CRITICAL, "ERROR: fota info point is NULL\n");
		return -1;		
	}
	...
	ptn = ptable_find(ptable, FOTAINFO_PART);
	if (ptn == NULL) {
		dprintf(CRITICAL, "ERROR: No %s partition found\n", FOTAINFO_PART);
		return -1;
	}

	offset += (FOTAINFO_BLOCK_A*blocksize + FOTAINFO_PAGE*pagesize);
	if (flash_read(ptn, offset, (void *)buf, pagesize)) {
		dprintf(CRITICAL, "ERROR: Cannot read fota info_main\n");
		return -1;
	}
	memcpy((void *)&info_main, buf, FOTAINFO_SIZE);
	...
	offset += (FOTAINFO_BLOCK_B*blocksize + FOTAINFO_PAGE*pagesize);
	if (flash_read(ptn, offset, (void *)buf, pagesize)) {
		dprintf(CRITICAL, "ERROR: Cannot read fota info_back\n");
		return -1;
	}
	memcpy((void *)&info_back, buf, FOTAINFO_SIZE);
...
/**
 * fota_info_check -- check full fota and A/B fota update state
 */
void fota_info_check(void)
{
	unsigned int fota_type;
	unsigned int fota_state;
	unsigned int fcount;
	dprintf(CRITICAL, "fota info check to recovery\n");
	get_fota_feature(&fota_type, &fota_state, &fcount);
	dprintf(CRITICAL, "type:%d state:%d fcount:%d\n", fota_type, fota_state, fcount);
	if (fota_type == FOTA_FULL) {
		if (fota_state == STAT_UPDATE || fota_state == STAT_BACKUP || 
			fota_state == STAT_FAILED && fcount < 5) {
...
void dump_fota_info(void)
{
	...
	if  (ret == 0)
	{
		dprintf(CRITICAL, "Magic:%s\n", info.magic);
		dprintf(CRITICAL, "Type:%d\n", info.type);
		dprintf(CRITICAL, "State:%d\n", info.eventstat[0]);
	}
}
