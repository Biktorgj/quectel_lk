...
#define DEFINE_SPMI_SET_POINTS(_name) \
struct spmi_voltage_set_points _name##_set_points = { \
	.name   = #_name, \
	.range	= _name##_ranges, \
	.count	= ARRAY_SIZE(_name##_ranges), \
}
...
found:
	...
	if (mapping->set_points) {
		...
		dprintf(INFO, "regulator name=%s type=0x%02X, subtype=0x%02X, dig major rev=0x%02X setpoints=%s\n",
			vreg->name, type, subtype, dig_major_rev, mapping->set_points->name);
	}
